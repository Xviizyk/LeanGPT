from lean4gen.curriculum import curriculum_batch, eval_batch, level_0, level_3, should_advance
...
def test_level_0_eval_offset_does_not_overlap_train():
    ...
    eval_ = set(level_0(20, seed_offset=10_000))
    assert train.isdisjoint(eval_)


def test_level_3_eval_offset_does_not_overlap_train():
    train = set(level_3(20, seed_offset=0))
    eval_ = set(level_3(20, seed_offset=10_000))
    assert train.isdisjoint(eval_)


def test_curriculum_batch_includes_lower_levels():
    batch = curriculum_batch(current_level=2, n_per_level=8)
    assert any("const_add" in s for s in batch)   # level 0
    assert any("add_comm" in s or "add_zero" in s or "zero_add" in s or "mul_one" in s for s in batch)  # level 1
    assert any("and_comm" in s or "or_comm" in s or "not_not" in s for s in batch)  # level 2
