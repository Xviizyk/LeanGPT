...
def test_iter_balanced_caps_per_signature(tmp_path):
    ...
    for i in range(10):
        store.add(
            f"theorem t{i}",
            f"theorem t{i} := by decide -- variant {i}",
            ok=True,
            has_sorry=False,
            errors=[],
        )
...
def test_iter_balanced_caps_per_statement(tmp_path):
    ...
    for i in range(10):
        store.add(
            "theorem same_thm",
            f"theorem same_thm := by simp -- v{i}",
            ok=True,
            has_sorry=False,
            errors=[],
        )
...
def test_stats_reports_success_rate(tmp_path):
    ...
    store.add(
        "theorem b", "theorem b := by bogus", ok=False, has_sorry=False, errors=["e"]
    )
    stats = store.stats()
