"""
Генератор кандидатов Lean4-кода на базе СВОЕЙ модели (LeanGPT),
обученной с нуля на собранном корпусе — без использования готовых LLM.
"""
...
class GenConfig:
    ...
    top_p: float = 0.95
    num_candidates: int = 8  # сколько кандидатов генерировать на один промпт
...
class LeanGenerator:
    def __init__(self, cfg: GenConfig) -> None:
        ...
        self.last_gen_stats: tuple[int, float] = (0, 0.0)  # (n_tokens, elapsed_sec) последнего generate()

    def load(self) -> None:
        self._tokenizer = ByteLevelBPETokenizer(
            f"{self.cfg.tokenizer_dir}/vocab.json", f"{self.cfg.tokenizer_dir}/merges.txt"
        )
    ...
    def generate(self, prompt: str) -> list[str]:
        """Вернуть список сгенерированных кандидатов (без префикса-промпта)."""
        ...
        n_new_tokens = (out.shape[1] - idx.shape[1]) * out.shape[0]  # суммарно по всем кандидатам в батче
...
def build_prompt(statement: str) -> str:
    """Промпт-шаблон: формулировка теоремы -> просим завершить доказательство.
    Т.к. модель обучена только на сыром коде (без инструкций), промпт —
    это просто начало Lean4-файла, которое модель должна продолжить."""
    return f"{statement} := "
