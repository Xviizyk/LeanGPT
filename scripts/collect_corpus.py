"""
Сбор корпуса .lean файлов из GitHub.

Стратегия:
1. Поиск репозиториев по ключевым словам ("language:Lean", "mathlib4" и т.п.)
   через GitHub Search API.
2. Скачивание архивов репозиториев (codeload.github.com), а не файл-за-файлом
   через API — иначе быстро упрёшься в rate limit.
3. Извлечение всех *.lean файлов в единый корпус.

Нужен GitHub token (даже с минимальными правами) — иначе rate limit
неавторизованных запросов очень маленький (60/час).
"""
...
def search_lean_repos(token: str, max_repos: int = 200) -> list[str]:
    """Вернуть список 'owner/repo' с Lean4-кодом, отсортированных по звёздам."""
    ...
    while len(repos) < max_repos:
        resp = requests.get(
            ...
            headers=headers,
            params={"q": "language:Lean", "sort": "stars", "order": "desc", "per_page": 100, "page": page},
            timeout=30,
        )
...
def download_and_extract(owner_repo: str, out_dir: Path, token: str) -> int:
    """Скачать tarball репозитория и вытащить из него все *.lean файлы.
