...
class DashboardState:
    ...
    tps_window: deque = field(default_factory=lambda: deque(maxlen=20))
...
class Dashboard:
    ...
    def _render(self) -> Table:
        ...
        success_rate = (
            self.state.total_proved / self.state.total_attempts
            if self.state.total_attempts
            else 0.0
        )
        ...
        table.add_row(
            "Попыток / доказано",
            f"{self.state.total_attempts} / {self.state.total_proved}",
        )
